// RobotBuilder Version: 1.5
//
// This file was generated by RobotBuilder. It contains sections of
// code that are automatically generated and assigned by robotbuilder.
// These sections will be updated in the future when you export to
// Java from RobotBuilder. Do not put any code or make any change in
// the blocks indicating autogenerated code or it will be lost on an
// update. Deleting the comments indicating the section will prevent
// it from being updated in the future.


package org.usfirst.frc692.RecycledRushRobot.subsystems;

import org.usfirst.frc692.RecycledRushRobot.Robot;
import org.usfirst.frc692.RecycledRushRobot.RobotMap;
import org.usfirst.frc692.RecycledRushRobot.commands.*;

import edu.wpi.first.wpilibj.*;
import edu.wpi.first.wpilibj.DoubleSolenoid.Value;
import edu.wpi.first.wpilibj.command.Subsystem;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;


/**
 *
 */
public class Elevator extends Subsystem {
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
    DigitalInput elevatorLevelGroundLimit = RobotMap.elevatorelevatorLevelGroundLimit;
    DigitalInput elevatorLevelStepLimit = RobotMap.elevatorelevatorLevelStepLimit;
    DigitalInput elevatorLevelTopLoadLimit = RobotMap.elevatorelevatorLevelTopLoadLimit;
    DoubleSolenoid flapperRelease = RobotMap.elevatorflapperRelease;
    DigitalInput elevatorSlowDownUp = RobotMap.elevatorelevatorSlowDownUp;
    DigitalInput elevatorSlowDownDown = RobotMap.elevatorelevatorSlowDownDown;
    SpeedController elevatorPlatformUpDownMotor = RobotMap.elevatorelevatorPlatformUpDownMotor;

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS

    
    // Put methods for controlling this subsystem
    // here. Call these from Commands.

    public void initDefaultCommand() {
        // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND
        setDefaultCommand(new elevatorTakeJoystickValie());

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND
	
        // Set the default command for a subsystem here.
        //setDefaultCommand(new MySpecialCommand());
    }
    private static final int movingUp = 1;
    private static final int movingDown = -1;
    private static final int groundPosition = 0;
    //these are constants to state that the elevator is moving in Up/Down direction 
    //AC 1/19/15
    
    private int elevatorState;
    //variable that is set to a constant to tell state
    //AC 1/19/15
    
    public void setElevatorMovingUp()
    {
    	elevatorState = movingUp;
    }
    //elevator is moving up
    //AC 1/19/15
    
    public void setElevatorMovingDown()
    {
    	elevatorState = movingDown;
    }
    //elevator is moving down
    //AC 1/19/15
    
    public void setGroundPosition()
    {
    	elevatorState = groundPosition;
    }

    //elevator is at done position
    //done position is intended floor
    //AC 1/19/15
    
    public boolean isMovingUp()
    {
    	if(elevatorState == 1)
    	{
    		return true;
    	}
    	else
    	{
    		return false;
    	}
    }
    //boolean to check if elevator is moving up 
    //true = up 
    //AC 1/19/15
    
    public boolean isMovingDown()
    {
    	if(elevatorState == -1)
    	{
    		return true;
    	}
    	else
    	{
    		return false;
    	}
    }
    //boolean to check if elevator is moving down
    //true = down
    //AC 1/19/15
    
    public void releaseFlap()
    {
    	//flapperRelease.set(Value.kForward);
    	flapperRelease.set(Value.kReverse);
    }
    // causes flapper to release totes
    // EV 1/15/2015
    
    public void resetFlap()
    {
    	flapperRelease.set(Value.kOff);
    }
    //air is turned off on flapper
    //flapper can now reset itself
    //AC 2/16/15
    
    public void forwardFlap()
    {
    	flapperRelease.set(Value.kForward);
    }
    
    
    public boolean onGroundFloor()
    {
    	if(elevatorLevelGroundLimit.get() == true)
    	{
    		SmartDashboard.putString("Ground Limit Switch:", "true");
    	}
    	else
    	{
    		SmartDashboard.putString("Ground Limit Switch:", "false");
    	}
    	return elevatorLevelGroundLimit.get();
    	
    }
    
    //returns true when pressed
    //indicates whether platform is on ground level
    //when pressed elevator stops
    //AC 1/15/15
    
    public boolean onStepLevel()
    {
    	if(elevatorLevelStepLimit.get() == true)
    	{
    		SmartDashboard.putString("Step Limit Switch:", "true");
    	}
    	else
    	{
    		SmartDashboard.putString("Step Limit Switch:", "false");
    	}
    	return elevatorLevelStepLimit.get();
    }
    //returns true when pressed
    //indicates whether platform is on step level
    //when pressed elevator stops
    //AC 1/16/15
    
    public boolean onTopLoadLevel()
    {
    	if(elevatorLevelTopLoadLimit.get() == true)
    	{
    		SmartDashboard.putString("Top Limit Switch:", "true");
    	}
    	else
    	{
    		SmartDashboard.putString("Top Limit Switch:", "false");
    	}
    	return elevatorLevelTopLoadLimit.get();
    }
    //returns true when pressed
    //when pressed elevator stops
    //indicates whether platform is on topLoad level
    //AC 1/16/15
    
    public void elevatorUp()
    {
    	elevatorPlatformUpDownMotor.set(1.0);
    }
    //elevator goes up when called at 100% speed
    //AC 1/16/15
    
    public void elevatorDown()
    {
    	elevatorPlatformUpDownMotor.set(-1.0);
    }
    //elevator platform goes down at 100% speed
    //AC 1/16/15
    
    public void elevatorStop()
    {
    	elevatorPlatformUpDownMotor.set(0.0);
    }
    //elevator platform stops
    //AC 1/16/15
    
    /*
     * Elevator Steps(Top --> Bottom):
     * Top Load
     * Step
     * Ground
     * AC 1/16/15
     */
    
    public void takeJoystickValue(double x)
    {
    	/*
    	if (RobotMap.stepLimitSwitchIsPressed){
    		// elevatorelevatorLevelStepLimit
    		
    		if (this.onStepLevel())
    		{
    			elevatorPlatformUpDownMotor.set(0.0);// do nothing
    			return;
    		}
    	}
    	*/
    	
    	if(Robot.elevator.onGroundFloor() && x < 0)
    	{
    		elevatorPlatformUpDownMotor.set(0.0);
    	}
    	else if(Robot.elevator.onTopLoadLevel() && x > 0)
    	{
    		elevatorPlatformUpDownMotor.set(0.0);
    	}
    	else 
    	{
    		elevatorPlatformUpDownMotor.set(x);
    	}
    }
    
    public void manualGoToStepLevel(double t)
    {
    	if(!Robot.elevator.onStepLevel())
    	{
    		elevatorPlatformUpDownMotor.set(t);
    	}
    	else 
    	{
    		elevatorPlatformUpDownMotor.set(0.0);
    	}
    }
    
    public boolean slowDownUp()
    {
    	return elevatorSlowDownUp.get();
    }
    
    public boolean slowDownDown()
    {
    	return elevatorSlowDownDown.get();
    }
    
    public void slowDownElevatorUp(double m)
    {
    	m = elevatorPlatformUpDownMotor.get();
    	if(Robot.elevator.slowDownUp())
    	{
    		elevatorPlatformUpDownMotor.set(m / 2);
    	}
    	else
    	{
    		elevatorPlatformUpDownMotor.set(m);
    	}
    }
    
    public void slowDownElevatorDown(double n)
    {
    	n = elevatorPlatformUpDownMotor.get();
    	// commented out the negative (???)
    	// EV 3/11/15
    	if(Robot.elevator.slowDownDown())
    	{
    		elevatorPlatformUpDownMotor.set(n / 2);
    	}
    	else
    	{
    		elevatorPlatformUpDownMotor.set(n);
    	}
    }
    
}
